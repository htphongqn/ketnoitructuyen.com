...
namespace Controller
{
    public class Home
    {
        ...
        public List<ESHOP_NEW> Loadindex(int type, int period, int limit)
        {
            try
            {
                var list = db.ESHOP_NEWs.Where(n => n.NEWS_PERIOD == period && n.NEWS_TYPE == type).Distinct().OrderByDescending(n => n.NEWS_PUBLISHDATE).OrderByDescending(n => n.NEWS_ORDER_PERIOD).Take(limit).ToList();
                return list;
            }
            catch (Exception)
            {
        ...
        public List<ESHOP_NEW> loadLoveOrSee(int type,int limit)
        {
            var list = db.ESHOP_NEWs.Where(n => n.NEWS_TYPE == 1&&(type==0 ? n.NEWS_DATESEE!=null : n.NEWS_DATE_LOVE!=null)).OrderByDescending(n => type == 0 ? n.NEWS_DATESEE : n.NEWS_DATE_LOVE).Take(limit).ToList();
            return list;
        }
        public List<ESHOP_NEW> loadGiovang(int type, int period, int limit)
        {
            var list = db.ESHOP_NEWs.Where(n => n.NEWS_PERIOD == period && n.NEWS_TYPE == type&&n.NEWS_UPDATE>=DateTime.Today&&n.NEWS_FROM_HOUR<=DateTime.Now.Hour&&n.NEWS_TO_HOUR>DateTime.Now.Hour).Distinct().OrderByDescending(n => n.NEWS_PUBLISHDATE).OrderByDescending(n => n.NEWS_ORDER_PERIOD).Take(limit).ToList();
            return list;
        }
        //Pro index all
        ...
        public List<ESHOP_NEW> Loadpro_new(int type,int limit)
        {
            try
            {
                var list =db.ESHOP_NEWs.Where(n=>n.NEWS_TYPE==type).Distinct().OrderByDescending(n => n.NEWS_PUBLISHDATE).OrderByDescending(n => n.NEWS_ORDER_PERIOD).Take(limit).ToList();
                return list;
            }
            catch (Exception)
            {
        ...
        public IQueryable<Pro_details_entity> Load_pro_index_cate(object catid,int skip,int limit)
        {
            try
            {
                ...
                var list = (from a in db.ESHOP_NEWS_CATs
                            ...
                            select new { b.NEWS_ID, b.NEWS_TITLE, b.NEWS_IMAGE3, b.NEWS_DESC, b.NEWS_SEO_URL, b.NEWS_PRICE2, b.NEWS_PRICE1, b.NEWS_URL, b.NEWS_ORDER_PERIOD, b.NEWS_PUBLISHDATE,b.NEWS_FIELD3, c.CAT_SEO_URL }).OrderByDescending(n => n.NEWS_PUBLISHDATE).OrderByDescending(n => n.NEWS_ORDER_PERIOD).Skip(skip).Take(limit).ToList();
                foreach (var i in list)
                {
                    Pro_details_entity pro = new Pro_details_entity();
                    pro.NEWS_ID = i.NEWS_ID;
