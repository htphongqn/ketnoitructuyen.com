...
namespace Controller
{
    public class Product_Details
    {
        ...
        #endregion
        public List<ESHOP_NEW> Load_Product_Detail(string _News_seo_url)
        {
            try
            {
                var list = db.ESHOP_NEWs.Where(n => n.NEWS_SEO_URL == _News_seo_url).ToList();
                return list;
            }
            catch (Exception)
            {
        ...
        public List<ESHOP_NEW> loadProcunghang(int hang,int limit)
        {
            var list = db.ESHOP_NEWs.Where(n => n.UNIT_ID2 == hang).Distinct().OrderByDescending(n=>n.NEWS_ID).Take(limit).ToList();
            return list;
        }
        // Show html
        ...
        public string getSeourl(int idth)
        {
            ...
            if (list.Count > 0)
            {
                return "/chuyen-muc/"+list[0].CAT_SEO_URL+".aspx";
            }
            return "";
        }
        ...
        public void Addsee(string _news_seo)
        {
            ...
            if (list.Count > 0)
            {
                if (String.IsNullOrEmpty(Utils.CStrDef(HttpContext.Current.Session["seeIP_" + _news_seo + ""])))
                {
                    ...
                    {
                        list[0].NEWS_COUNT = 1;
                        list[0].NEWS_DATESEE = DateTime.Now;
                        db.SubmitChanges();
                    }
                    else
                    {
        ...
        public void Addlove(string _news_seo)
        {
            var list = db.ESHOP_NEWs.Where(n => n.NEWS_SEO_URL == _news_seo).ToList();
            if (list.Count > 0)
            {
                list[0].NEWS_DATE_LOVE = DateTime.Now;
                db.SubmitChanges();
            }
